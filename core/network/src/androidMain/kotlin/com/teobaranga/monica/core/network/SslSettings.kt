    ...
    override suspend fun deleteCertificate(sha256Hash: ByteString) {
        withContext(dispatcher.io) {
            ...
            val certAlias = keyStore.aliases().asSequence()
                .firstOrNull {
                    val x509Certificate = keyStore.getCertificate(it) as? java.security.cert.X509Certificate
                    sha256Hash.toByteArray().contentEquals(x509Certificate?.sha256Hash()?.toByteArray())
                }
            if (certAlias != null) {
                logging.i { "Deleting certificate: $certAlias" }
                keyStore.deleteEntry(certAlias)
                val file = File(context.filesDir, "keystore.jks")
                keyStore.store(file.outputStream().buffered(), "foobar".toCharArray())
                isStale = true
